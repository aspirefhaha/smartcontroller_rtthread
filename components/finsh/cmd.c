...
long test_mem(int size)
{
	char * testm = rt_malloc(size);
	rt_kprintf(" get addr %x\n",testm);
	list_memheap();
	if(testm)
		rt_free(testm);
	return 0;
}  
FINSH_FUNCTION_EXPORT(test_mem, test mem heap);
...
long ls_memheap(void)
{
	struct rt_memheap *mh;
	struct rt_memheap_item * item;
    struct rt_list_node *node;
	struct rt_list_node *list = &rt_object_container[RT_Object_Class_MemHeap].object_list;
	node = list->prev;
    
    do{
        mh = (struct rt_memheap *)rt_list_entry(node, struct rt_object, list);
		item = mh->	block_list;
        rt_kprintf("memheap %-8.*s addr %x ,pool size %-010d max_used %-013d available size%-05d\n",
                   RT_NAME_MAX,
                   mh->parent.name,
				   mh->start_addr,
                   mh->pool_size,
                   mh->max_used_size,
                   mh->available_size);
		rt_kprintf("\t\t block_list %x magic %x pool %x next %x prev %x free_next %x free_prev %x\n",
				item,item->magic,item->pool_ptr,item->next,item->prev,item->next_free,item->prev_free);
		item = mh->free_list;
		rt_kprintf("\t\t free_list %x magic %x pool %x next %x prev %x free_next %x free_prev %x\n",
				item,item->magic,item->pool_ptr,item->next,item->prev,item->next_free,item->prev_free);
		do{
			item = item->next_free;
			rt_kprintf("\t\t next free_list %x magic %x pool %x next %x prev %x free_next %x free_prev %x\n",
					item,item->magic,item->pool_ptr,item->next,item->prev,item->next_free,item->prev_free);
		}while(item->next_free != &mh->free_header && item->magic == 0x1ea01ea0);
		item = &mh->free_header;
		rt_kprintf("\t\t free_header %x magic %x pool %x next %x prev %x free_next %x free_prev %x\n",
				item,item->magic,item->pool_ptr,item->next,item->prev,item->next_free,item->prev_free);
		node = node->prev;
...
FINSH_FUNCTION_EXPORT(ls_memheap, list memory heap in system)
