...
#include <rtthread.h>
#include <string.h>
#define assert(str) assert_param(str)
#define assert_param(str)	do{}while(0)
#define malloc rt_malloc
#define free rt_free
#define _tcslen(str)	rt_strlen(str)
#define _tcsnicmp(stra,strb,cblen)	rt_strncmp(stra,strb,cblen)
#define _tcsstr	rt_strstr
#define McbStrdup(pstr,cbToken) rt_strndup(pstr,cbToken)
#define _tcscpy	rt_strcpy
...
#ifndef _WINDOWS
/**
 * This function will compare two strings while ignoring differences in case  with specified maximum length
 *
 ...
 * @return the result
 */
rt_base_t rt_strncasecmp(const char *a, const char *b, rt_ubase_t count)
{
	int ca, cb;
...
#define rt_strcpy(a,b)	rt_strncpy(a,b,strlen(b))


char *rt_strndup(const char *s,rt_size_t size)
{
	rt_size_t len = rt_strlen(s) + 1;
	char * tmp = NULL;
	if(len > size)
		len = size;
	tmp = (char *)rt_malloc(len+1);
...
McbXMLElement * McbCreateRoot()
{
	...
	McbInitElement(pElement, NULL, 0, TRUE);
					...
					LPTSTR lpszValue, int nGrowBy)
{
    ...
    */
	...
	if(lpszValue[0] != '\"'){
		char * ptmpValue = (char * ) rt_malloc(rt_strlen(lpszValue)+3);
		rt_sprintf(ptmpValue,"\"%s\"",lpszValue);
	 	rt_free(lpszValue);
		lpszValue = ptmpValue;
...
#define McbLENSTR(lpsz) (lpsz ? _tcslen(lpsz) : 0)
    ...
    */
	...
	if (cbElement)
	{
        ...
        */
		...
		//rt_kprintf("name %s isDlr %d\n",pEntry->lpszName,pEntry->nIsDeclaration);
        if (lpszMarker)
        {
            if (cb)
            {
    ...
    */
	...
	if (cbElement && !pEntry->nIsDeclaration)
	{
       /*
        *********************************************************************
