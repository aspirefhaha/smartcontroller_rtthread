...
#ifdef RT_USING_RTGUI
	else if(GPDMA_IntGetStatus(GPDMA_STAT_INT, DMA_TP_CHANNEL))
	{						 
		// Check counter terminal status
		if(GPDMA_IntGetStatus(GPDMA_STAT_INTTC, DMA_TP_CHANNEL))
		{
			// Clear terminate counter Interrupt pending
			GPDMA_ClearIntPending (GPDMA_STATCLR_INTTC, DMA_TP_CHANNEL);

			Channel_TP_TC++;
		}

		// Check error terminal status
		if (GPDMA_IntGetStatus(GPDMA_STAT_INTERR, DMA_TP_CHANNEL))
		{
			// Clear error counter Interrupt pending
			GPDMA_ClearIntPending (GPDMA_STATCLR_INTERR, DMA_TP_CHANNEL);
