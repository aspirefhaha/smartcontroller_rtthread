...
/*********************************************************************//**
 * @brief		Write data to specific port pin in Byte mode
 * 
 * @param[in]	portNum		Port number, in range from 0 to 4
 * @param[in]	byteNum		Byte part number, should be in range from 0 to 3
 * @param[in]	Value		Date to be written
 * @return		None
 * Note: 
 * 
 **********************************************************************/
void GPIO_ByteWritePort(uint8_t portNum, uint8_t byteNum, uint8_t Value)
{
    GPIO_Byte_TypeDef *pFIO = FIO_ByteGetPointer(portNum);
    if ((byteNum >= 0) && (byteNum <= 3))
	{
		pFIO->FIOPIN[byteNum] = Value;
	}
}
