...
static void test_gui_entry(void *parameter)
{
...
long application_init(void)
{
    ...
    if (inited == RT_FALSE) /* 避免重复初始化而做的保护 */
    {
        ...
        tid = rt_thread_create("test_gui",
                               test_gui_entry, RT_NULL,
                               2048 * 2, 25, 10);
...
/* 显示对话框后的定时器超时回调函数，和RT-Thread中的timer不相同，这个超时函数的执行上下文是显示对话框线程的上下文 */
 void test_gui_diag_close(struct rtgui_timer* timer, void* parameter)
 {
...
FINSH_FUNCTION_EXPORT(application_init, test_gui on gui) 
FINSH_FUNCTION_EXPORT(emouse,enumulator mouse)
