...
static void create_normal_win(void)
{
    ...
    normal_window = rtgui_win_create(RT_NULL, "ANorWin",
    ...
    rt_sprintf(normal_window_label_text,
               "display %d times", normal_window_show_count);
...
static char *get_win_title()
{
    ...
    rt_sprintf(win_title, "Window %d", ++win_no);
    return win_title;
}
...
static void demo_modalwin_onbutton(struct rtgui_object *object, rtgui_event_t *event)
{
    ...
    label = rtgui_label_create("This is a Modal Win");
    rtgui_widget_set_rect(RTGUI_WIDGET(label), &rect);
...
static void demo_ntitlewin_onbutton(struct rtgui_object *object, rtgui_event_t *event)
{
    ...
    /* 创建一个文本标签 */
    label = rtgui_label_create("NoBorder");
    rtgui_font_get_metrics(RTGUI_WIDGET_FONT(label), "NoBorderWin", &widget_rect);
...
rtgui_container_t *demo_view_window(void)
{
    ...
    rect.y2 = rect.y1 + 20;
    /* 创建按钮用于显示正常窗口 */
    button = rtgui_button_create("普通窗口");
