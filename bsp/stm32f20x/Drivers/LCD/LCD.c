...
#define LCD_REG              (*((volatile unsigned short *) 0x6D000000)) /* RS = 0 */
#define LCD_RAM              (*((volatile unsigned short *) 0x6D010000)) /* RS = 1 */
...
* Attention		 : None
*******************************************************************************/
rt_inline void LCD_SetPoint(uint16_t Xpos,uint16_t Ypos,uint16_t point)
{
...
//fhaha add
static void mylcd_setpixel(const char *pixel, int x, int y)
{
	LCD_SetPoint((uint16_t)x,(uint16_t)y,*(uint16_t*)pixel);
}

static 	void mylcd_getpixel(char *pixel, int x, int y)
{
 	*(uint16_t *)pixel=LCD_GetPoint((uint16_t)x,(uint16_t)y);
}

static void mylcd_drawhline(const char *pixel, int x1, int x2, int y)
{
 	LCD_DrawLine((uint16_t) x1, (uint16_t) y, (uint16_t) x2, (uint16_t) y , *(uint16_t*)pixel );
}
static void mylcd_drawvline(const char *pixel, int x, int y1, int y2)
{
 	LCD_DrawLine((uint16_t) x, (uint16_t) y1, (uint16_t) x, (uint16_t) y2 , *(uint16_t*)pixel );
}

static void mylcd_blitline(const char *pixel, int x, int y, rt_size_t size)
{
...
{
	mylcd_setpixel,	//void (*set_pixel) (const char *pixel, int x, int y);
	mylcd_getpixel,	//void (*get_pixel) (char *pixel, int x, int y);
	mylcd_drawhline,	//void (*draw_hline)(const char *pixel, int x1, int x2, int y);
	mylcd_drawvline,	//void (*draw_vline)(const char *pixel, int x, int y1, int y2)
	mylcd_blitline,	//void (*blit_line) (const char *pixel, int x, int y, rt_size_t size);
			/*画水平杂色线段。上面介绍的画线都是画单色线的，如果你想让线段的每个点的颜色都不同就可用这个接口，
  				pixels就是像素值数组的首地址、size为这个数组的大小同时也是这个线段的长度、x,y为线段的起点。  */
};
